...
export default class Service {
  ...
  process = {
    ...
    fetchPages: async ({
      ...
      try {
        for (const url of (Array.isArray(urls) ? urls : [])) {
          const textHtml = await this.#getTradePageHtml(url, { signal: abortController.signal })
    ...
    restoreGroup: async ({
      ...
      try {
        const textHtml = await this.#postHtml(url, payload, { signal: abortController.signal })
  ...
  #getTradePageHtml = async (url, { signal } = {}) => {
  ...
  #postHtml = async (url, payload, { signal } = {}) => {
  ...
  #setPageSize = async (url, payload, { signal } = {}) => this.#postHtml(url, payload, { signal })
}
