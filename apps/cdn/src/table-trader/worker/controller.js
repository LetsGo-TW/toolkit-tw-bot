...
const deserializeWorkerError = (error = {}) => {
  const runtimeError = new Error(error?.message || "table-trader worker failed")
...
export default class Controller {
  ...
  async restoreGroup({ url = "", payload = null, onOcurrenceUpdate = () => {} } = {}) {
    return await this.#request({
      type: "restoreGroup",
      url,
      payload,
  ...
  #configureRuntime() {
    if (this.#worker) {
      ...
      this.#worker.onerror = (event) => {
        this.#handleRuntimeError(event?.error || new Error(event?.message || "table-trader worker runtime error"))
      }
      return
    }
  ...
  #request({
    ...
    return new Promise((resolve, reject) => {
      ...
      if (typeof handler !== "function") {
        ...
        reject(new Error(`Unknown table-trader worker process: ${type}`))
        return
      }
  ...
  #handleRuntimeError(error) {
    const runtimeError = error instanceof Error
      ? error
      : new Error(String(error || "table-trader runtime error"))
