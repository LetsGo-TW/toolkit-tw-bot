...
export async function getTargeSelection(x, y, { signal } = {}) {
  const gameData = getGameData();
  const url = new URL(
    `${gameData.link_base_pure}api&ajax=target_selection&input=${x}|${y}&type=coord&request_id=1&limit=7&offset=0`,
    window.origin
  );
  ...
  try {
    ...
    return response?.villages?.[0] || null;
  } finally {
    clearTimeout(t);
  }
}
