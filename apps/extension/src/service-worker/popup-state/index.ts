...
export async function getPopupState(request: PopupStateRequest = {}) {
  ...
  return {
    ...
    points: normalizeNumberish(pageState?.points ?? tabContext?.points),
    rank: normalizeNumberish(pageState?.rank ?? tabContext?.rank),
    villages: normalizeNumberish(pageState?.villages ?? tabContext?.villages),
